...
from logic.alertas import New_Email
...
class MonitorService:
    def __init__(self):
        ...
        self.urls_alertadas = set()

    async def chequear_url(self, url: str):
        inicio = datetime.now()
        try:
        ...
        if estado == "offline" and url not in self.urls_alertadas:
            New_Email(url, codigo_http)
            self.urls_alertadas.add(url)

    async def chequear_todas(self):
        print(f"\n🔍 Chequeando {len(self.URLS)} URLs — {datetime.now().strftime('%H:%M:%S')}")
        await asyncio.gather(*[self.chequear_url(url) for url in self.URLS])